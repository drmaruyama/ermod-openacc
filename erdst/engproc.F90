...
751          format(i5, 9999f15.5)
    ...
    write(eng_io, '(A2,A13,A15,A5,A25,A25)') "# ", &
         "bin_left", "bin_middle", "spec", "ratio", "dens"
    do iduv = 1, ermax
       call representative_bin_info(iduv, leftbin, middlebin, invwt, pti)
       write(eng_io, '(g15.7,g15.7,i5,g25.15,g25.15)') &
            leftbin, middlebin, pti, edens(iduv), edens(iduv) * invwt
    enddo
    endfile(eng_io)
    close(eng_io)
    !
  ...
  subroutine update_histogram(stat_weight_solute, uvengy0, uvengy, cnt)
    ...
    case(YES)
       ...
       factor = uvengy0(cnt) - voffset  ! shifted by offset
       select case(slttype)
       case(SLT_SOLN)
    ...
    if (selfcal == YES) then
       call getiduv(0, uvengy0(cnt), iduv)
       eself(iduv) = eself(iduv) + engnmfc
    endif
    minuv(0) = min(minuv(0), uvengy0(cnt))
    maxuv(0) = max(maxuv(0), uvengy0(cnt))
